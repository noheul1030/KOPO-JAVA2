...
public class 이마트Method {

	// String 배열 값 30개 대입
	static String[] kopo11_itemname = { "오뚜기 진비빔면 156g", "피코크 들깨미역국500", "GAP 죽장사과 4-6입", "순 유기농 바나나(봉)",
					"해피홈 SAFE365 핸드", "(G)LH원형유리화병 10", "상투과자E", "더블링 나일론 깡 롤", "해피홈 SAFE 핸드워시", "농심 신라면120gx5", "(대)국내산삼겹살찌개",
					"피코크 탄두리 닭가슴살", "(달콤)순살닭강정(g)", "1+ 등급란 10개입 특", "동원양반볶음김치기획", "테일러 푸룬주스", "DZ주니어양말3족C_BK", "DZ주니어양말3족C_WH",
					"프리미엄생연어초밥", "피코크 들깨미역국500", "남양 맛있는우유GT 저지방", "녹차원흑당시럽10입", "푸르밀 미숫가루우유", "자일로스갈색설탕500g", "CJ 비비고칩 20G",
					"흑원당 말차 밀크티", "울샴푸 퍼플라일락", "앤더슨프레첼708g", "진라면소컵(매)65gx6", "케라시스 앰플 트리트먼트" };
	
	static int kopo11_taxpass = 0; // int 변수에 초기값 0 대입 (* 면세 물품 금액)
	static int kopo11_sum = 0; // int 변수에 초기값 0 대입 (합계)
	static int kopo11_point = 5473; // int 변수에 누적 포인트 5473 대입
	
	static DecimalFormat kopo11_df = new DecimalFormat("###,###,###,###,###"); // 값을 지정한 형식으로 format해주는 객체 생성
//	static SimpleDateFormat kopo11_sd = new SimpleDateFormat("yyyy-MM-dd HH:mm"); // 시간을 내가 지정한 틀로 format해서 뽑아주는 객체 생성
	
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	public static void main(String[] args) { // 메인
		
		int kopo11_itemCount = kopo11_itemname.length; // int 변수에 전역변수의 길이값 대입
		TitlePrint(); // 영수증 맨위 타이틀 인쇄 메서드
		System.out.printf("[구매]%s%19s\n", TimeStemp(), "POS:0011-9861"); // 구매날짜 오늘정보 String 출력 개행
		HeaderPrint(); // 헤더(상품명,단가,수량,금액) 인쇄 메서드
		
		for (int kopo11_i = 0; kopo11_i < kopo11_itemCount; kopo11_i++) { // 0 ~ kopo11_itemCount 만큼 도는 반복문
			ItemPrint(kopo11_i); // 항목 갯수만큼 항목 인쇄 메서드에 i값 파라미터 전달
		}
		TotalPrint(); // 합계 세액등을 인쇄 메서드
	
	}

	public static void TitlePrint() { // 영수증 맨위 타이틀 인쇄
	...
	public static String TimeStemp() { // 출력 시간 인쇄
		SimpleDateFormat kopo11_sd = new SimpleDateFormat("yyyy-MM-dd HH:mm"); // 시간을 내가 지정한 틀로 format해서 뽑아주는 객체 생성
		String Time = (kopo11_sd.format(new Date())); // String 변수에 format한 정보 대입
		return Time; // Time을 리턴한다
	}

	public static void HeaderPrint() { // 헤더(상품명,단가,수량,금액) 인쇄
		System.out.printf("-----------------------------------------\n"); // --------- 라인 출력 개행
		System.out.printf("%6s%13s%5s%8s\n", "상품명", "단가", "수량", "금액"); // String 출력 개행
		System.out.printf("-----------------------------------------\n"); // --------- 라인 출력 개행

		}

	public static void ItemPrint(int kopo11_i) { // 항목 갯수만큼 항목 인쇄

		// int 배열 값 30개 대입
		int[] kopo11_price = { 1980, 3980, 4480, 3480, 7980, 7500, 6900, 5980, 3300, 5500, 3380, 6610, 2750, 10800,
				7130, 3980, 4480, 4980, 3210, 6950, 2980, 1990, 14980, 11780, 8980, 4980, 2480, 7920, 4980, 4980 };

		// int 배열 값 30개 대입
		int[] kopo11_num = { 2, 1, 1, 1, 2, 1, 1, 1, 2, 1, 1, 3, 1, 1, 2, 1, 1, 1, 2, 1, 2, 1, 1, 1, 1, 2, 1, 2, 1,
				2 };

		// 면세 유무 체크 boolean 배열 값 30개 대입
		boolean[] kopo11_taxfree = { true, false, false, false, false, false, true, false, true, true, false, false,
				false, false, false, false, true, false, false, true, false, false, true, false, false, true, false, true,
				false, false };
		...
		System.out.printf("%12s", kopo11_df.format(kopo11_price[kopo11_i])); // String 출력 개행
		System.out.printf("%3s", kopo11_num[kopo11_i]); // String 출력 개행
		System.out.printf("%11s\n", kopo11_df.format(kopo11_pricenum)); // String 출력 개행

	}
	

	public static void TotalPrint() { // 합계 세액등을 인쇄
		double kopo11_tax = ((double) kopo11_sum - kopo11_taxpass) / 11.0; // double 변수에 부가세 계산값 대입

		if (kopo11_tax == (int) kopo11_tax) // double tax값과 int로 소숫점 버린 값이 같으면
			kopo11_tax = (int)kopo11_tax; //  tax = (int)tax
		else // double tax값과 int로 소숫점 버린 값 같지 않으면
			kopo11_tax = (int)kopo11_tax+1; // 부과세 +1 값 대입
