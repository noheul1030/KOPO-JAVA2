...
public class PracticeL08 {
	...
	public static void main(String[] args) throws IOException {
		...
		while ((kopo11_readtxt = kopo11_br.readLine()) != null) { // readtxt의 값이 null이 아니면 도는 반복문
			...
			String[] kopo11_field = kopo11_readtxt.split("%_%"); // String 배열에 %_%을 기준으로 구분하여 저장

			// field의 길이가 2보다 크고 '그리고' field[2]번째 값의 ^을 ""로 replace,trim 공백제거 후 
			// substring(0, 7) 값이 "A005930"와 일치하면
			if (kopo11_field.length > 2 && kopo11_field[2].replace("^", "").trim().substring(0, 7).equals("A005930")) {
		...
		// cnt = 전체 데이터의 건수, wcnt = 조건에 맞춰서 뽑아낸 건수
