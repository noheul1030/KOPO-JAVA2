...
//		String kopo11_itemname1 = "퓨어에어 비말차단용마스크(최고급형)";	// String 변수에 "퓨어에어 비말차단용마스크(최고급형)" 대입
		...
		int kopo11_amount1 = 4; // int 변수에 1 대입

		String kopo11_itemname2 = "자일로스갈색설탕500g"; // String 변수에 "슬라이드식명찰(가로형)(100호)" 대입
//		String kopo11_itemname2 = "슬라이드식명찰(가로형)(100호)"; // String 변수에 "슬라이드식명찰(가로형)(100호)" 대입
		...
		int kopo11_price2 = 5000; // int 변수에 1000 대입
		int kopo11_amount2 = 2; // int 변수에 1 대입
...
//		String kopo11_itemname3 = "매직흡착 인테리어후크(알루미늄타입)"; // String 변수에 "매직흡착 인테리어후크(알루미늄타입)" 대입
		...
		int kopo11_price3 = 24000; // int 변수에 1000 대입
		int kopo11_amount3 = 2; // int 변수에 1 대입

		// int 변수에 물건 가격 * 수량 값 대입
		int kopo11_priceSum = (kopo11_price1 * kopo11_amount1) + (kopo11_price2 * kopo11_amount2)
				+ (kopo11_price3 * kopo11_amount3);

		double kopo11_tax = (double) kopo11_priceSum / 11.0; // double 변수에 부가세 계산값 대입
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		String kopo11_str1 = subStrByte(kopo11_itemname1, 17); // kopo11_itemname1의 Byte 17까지만 출력
		if (kopo11_itemname1.getBytes().length < 17) { // itemname1의 byte 크기가 17보다 작으면
			System.out.printf("%s", kopo11_itemname1); // itemname1 출력 개행
			for (int j = 0; j < 17 - kopo11_itemname1.getBytes().length; j++) { // 17 - itemname1의 byte 크기만큼
				System.out.printf("%s", " "); // 공백찍기
			}
		} else
			System.out.printf("%s", kopo11_str1); // itemname1의 byte 크기가 17보다 크면 그냥 출력

		// 1. 가격 , 수량 , 가격 * 수량 출력 개행
		System.out.printf("%12s", kopo11_df.format(kopo11_price1));
		System.out.printf("%3s", kopo11_amount1);
		System.out.printf("%8s\n", kopo11_df.format(kopo11_price1));

		System.out.printf("[%s]\n", kopo11_itemcode1); // String kopo11_itemcode1 출력 개행

		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

		String kopo11_str2 = subStrByte(kopo11_itemname2, 17); // kopo11_itemname2의 Byte 15까지만 출력
		if (kopo11_itemname2.getBytes().length < 17) { // itemname2의 byte 크기가 17보다 작으면
			System.out.printf("%s", kopo11_itemname2); // itemname2 출력 개행
			for (int j = 0; j < 17 - kopo11_itemname2.getBytes().length; j++) { // 17 - itemname2의 byte 크기만큼
				System.out.printf("%s", " "); // 공백찍기
			}
		} else
			System.out.printf("%s", kopo11_str2); // itemname2의 byte 크기가 17보다 크면 그냥 출력

		// 2. 가격 , 수량 , 가격 * 수량 출력 개행
		System.out.printf("%12s", kopo11_df.format(kopo11_price2));
		System.out.printf("%3s", kopo11_amount2);
		System.out.printf("%8s\n", kopo11_df.format(kopo11_price2));
		System.out.printf("[%s]\n", kopo11_itemcode2); // String kopo11_itemcode2 출력 개행

		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

		String kopo11_str3 = subStrByte(kopo11_itemname3, 17); // kopo11_itemname3의 Byte 17까지만 출력
		if (kopo11_itemname3.getBytes().length < 17) { // itemname3의 byte 크기가 15보다 작으면
			System.out.printf("%s", kopo11_itemname3); // itemname3 출력 개행
			for (int j = 0; j < 17 - kopo11_itemname3.getBytes().length; j++) { // 17 - itemname3의 byte 크기만큼
				System.out.printf("%s", " "); // 공백찍기
			}
		} else
			System.out.printf("%s", kopo11_str3); // itemname3의 byte 크기가 17보다 크면 그냥 출력

		// 3. 가격 , 수량 , 가격 * 수량 출력 개행
		System.out.printf("%12s", kopo11_df.format(kopo11_price3));
		System.out.printf("%3s", kopo11_amount3);
		System.out.printf("%8s\n", kopo11_df.format(kopo11_price3));
		...
		System.out.printf("%16s%21s\n", "부가세",
				kopo11_df.format((int) (kopo11_priceSum - (kopo11_priceSum - kopo11_tax)))); // 부가세 출력 개행
		System.out.printf("----------------------------------------\n"); // ------------ 라인 출력 개행
		...
		if (!kopo11_str.isEmpty()) { // str이 비어있지 않을 때 = false
			...
			if (kopo11_str.getBytes().length <= kopo11_num) { // str의 byte길이가 cutlen 이하일때
				...
				for (char kopo11_ch : kopo11_str.toCharArray()) { // str.toCharArray()만큼 도는 반복문
					kopo11_nCnt += String.valueOf(kopo11_ch).getBytes().length; // nCnt에
																				// String.valueOf(ch).getBytes().length
																				// 누적 더하기
